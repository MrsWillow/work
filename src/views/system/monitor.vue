<template>
  <div class="app-container">
    <el-row :gutter="20">
          ...
          <div class="el-table el-table--enable-row-hover el-table--medium">
            <table cellspacing="0" style="width: 100%;">
              ...
              <tr>
                ...
                <td><div class="cell" v-if="server.mem">{{ server.mem.total }}M</div></td>
              </tr>
              <tr>
                <td><div class="cell">已用内存</div></td>
                <td><div class="cell" v-if="server.mem">{{ server.mem.used}}M</div></td>
              </tr>
              <tr>
                <td><div class="cell">剩余内存</div></td>
                <td><div class="cell" v-if="server.mem">{{ server.mem.free }}M</div></td>
              </tr>
              <tr>
                <td><div class="cell">使用率</div></td>
                <td><div class="cell" v-if="server.mem" :class="{'text-danger': server.mem.usage > 80}">{{ server.mem.usage }}%</div></td><td><div class="cell" v-if="server.jvm" :class="{'text-danger': server.jvm.usage > 80}">{{ server.jvm.usage }}%</div></td>
              </tr>
              </tbody>
            </table>
          </div>
      ...
      <el-col :span="24" :style="{ marginTop: cardBox}">
        <el-card>
          <div slot="header">
            <span>服务器信息</span>
          </div>
          ...
          <div class="el-table el-table--enable-row-hover el-table--medium">
            <table cellspacing="0" style="width: 100%;">
              ...
              <tr>
                ...
                <th class="is-leaf"><div class="cell">总大小(MB)</div></th>
                <th class="is-leaf"><div class="cell">可用大小(MB)</div></th>
                <th class="is-leaf"><div class="cell">已用大小(MB)</div></th>
    ...
    methods:{
      showSystemInfo:function () {
        ...
        .then(function (response) {
          ...
          that.server = response.data.data
        })
      }
    }
  }
